...
// species editing:
    ...
    private boolean brushAbioticMode = false;
    ...
    private final Map<String, GridPane> speciesPanelEditors = new HashMap<>();
...
//*********** GENERAL METHODS ****************\\
    ...
    private ToolBar buildToolbar(String mode) {
        ...
        Label title = new Label("Run Mode: " + mode);
        if (isTestGrid){
            title = new Label("Test Mode: " + mode);
        }
    ...
    private void updateSpeciesParameterUI() {
    // Loop through each species and update its parameter panel
    for (Species species : currentResult.speciesList()) {
        // Get the existing panel for this species
        GridPane existingPanel = speciesPanelEditors.get(species.getName());
        
        if (existingPanel != null) {
            // Get current parameter values from the restored species
            float[] parameterValues = {
                species.getSunlightC(), species.getSunlightR(), 
                species.getMoistureC(), species.getMoistureR(), 
                species.getTemperatureC(), species.getTemperatureR(), 
                species.getSlopeC(), species.getSlopeR(), 
                species.getMaxHeightOpen(), species.getMaxHeightClosed(), 
                species.getQ(), species.getLifeSpan()
            };
            
            // Update each text field in the existing panel
            // The text fields are at column 1 of each row
            int paramIndex = 0;
            for (Node node : existingPanel.getChildren()) {
                if (node instanceof TextField && paramIndex < parameterValues.length) {
                    TextField textField = (TextField) node;
                    // Only update TextFields in column 1 (the value fields)
                    if (GridPane.getColumnIndex(node) != null && GridPane.getColumnIndex(node) == 1) {
                        textField.setText(String.valueOf(parameterValues[paramIndex]));
                        paramIndex++;
                    }
                }
            }
    ...
    // MODIFIED: Fixed the issue with listener using incorrect variable scope
    private GridPane createSpeciesPanelEditor(Species species)
    {
        GridPane speciesPanelEditor = new GridPane();
        ...
        for (int i = 0; i < speciesParameters.length; i++) {
            final int index = i;  // final copy for lambda - MOVED inside loop
            ...
            parameterValue.textProperty().addListener((obs, oldText, newText) -> {
                if (!newText.trim().isEmpty()) {
                    try {
                        ...
                        // Update the species object based on the parameter index
                        switch (index) {
                            case 0: species.setSunlightC(value); break;
                            case 1: species.setSunlightR(value); break;
                            case 2: species.setMoistureC(value); break;
                            case 3: species.setMoistureR(value); break;
                            case 4: species.setTemperatureC(value); break;
                            case 5: species.setTemperatureR(value); break;
                            case 6: species.setSlopeC(value); break;
                            case 7: species.setSlopeR(value); break;
                            case 8: species.setMaxHeightOpen(value); break;
                            case 9: species.setMaxHeightClosed(value); break;
                            case 10: species.setQ(value); break;
                            case 11: species.setLifeSpan(value); break;
                        }
        ...
        speciesPanelEditors.put(species.getName(), speciesPanelEditor);
    ...
    public Button updateSpeciesParametersButton()
    {
        ...
        updateSpeciesParametersButton.setOnAction(e -> {
            ...
            if (isTestGrid) {
                // Rebuild the parameter panel to refresh the UI
                mainContent.getChildren().remove(mainContent.getChildren().size() - 1);
                mainContent.getChildren().add(buildParameterPanelTest());
                updateGridEditors();
            }
            else
            {
                mainContent.getChildren().remove(mainContent.getChildren().size() - 1);
                mainContent.getChildren().add(buildParameterPanelRun());
            }
    ...
    public void setCurrentResult(SimulationResult result){
...
//*********** RUN VIEW METHODS ****************\\
    ...
    private VBox buildParameterPanelRun(){
        ...
        for (Species species : simulationEngine.getSpeciesList())
        {
            ...
            speciesPanelEditors.put(species.getName(), speciesParametersPanel);

            TitledPane speciesHeader = new TitledPane(species.getName(), speciesPanelEditors.get(species.getName()));
            speciesHeader.setExpanded(false);
            speciesHeader.setStyle("-fx-background-color: " +species.getColour() + "; -fx-text-fill: "+ species.getColour()+ ";");
    ...
    private TitledPane createAbioticsPanel()
    {
        ...
        brushSizeSlider.setPrefWidth(400);
        ...
        tabs.getSelectionModel().selectedItemProperty().addListener((obs, oldTab, newTab) -> {
            if (newTab != null) {
                ...
                console.log("If any previous brush mode was toggled on, now toggled off.");
                switch(newTab.getText()){
                    case("Environment (Elevation)"):
                        ...
                        case("Forest"):
                            forestView.draw();
                            System.out.println("Brush functions are disabled for Forest tab. Please switch tab if you require the brush utility.");
                            break;
                }
            }
        });
        ...
        enableBrushRemovalBtn.setPrefWidth(400);
        ...
        enableAgeChangeBtn.setOnAction(e -> 
            ...
            switch(tabs.getSelectionModel().getSelectedItem().getText()){
                    case("Environment (Elevation)"):
                        forestElevationView.setSelectedSpeciesSupplier(getSelectedSpecies);
                        brushAge(forestElevationView, brushSizeSlider, ageSlider, false);
                        break;
                    case("Temperature"):
                        forestTemperatureView.setSelectedSpeciesSupplier(getSelectedSpecies);
                        brushAge(forestTemperatureView, brushSizeSlider, ageSlider, false);
                        break;
                    case("Sunlight"):
                        forestSunlightView.setSelectedSpeciesSupplier(getSelectedSpecies);
                        brushAge(forestSunlightView, brushSizeSlider, ageSlider, false);
                        break;
                    case("Moisture"):
                        forestMoistureView.setSelectedSpeciesSupplier(getSelectedSpecies);
                        brushAge(forestMoistureView, brushSizeSlider, ageSlider, false);
                        break;
                    case("Age"):
                        forestAgeView.setSelectedSpeciesSupplier(getSelectedSpecies);
                        brushAge(forestAgeView, brushSizeSlider, ageSlider, true);
                        break;
                    case("Pink Noise"):
                        break;
                    case("Forest"):
        ...
        enableAgeChangeBtn.setPrefWidth(400);

        Button enableAbioticChangeBtn = new Button("Toggle Brush Abiotic Mode");
        enableAbioticChangeBtn.setOnAction(e -> 
            {
            brushAgeMode = false;
            brushRemovalMode = false;
            if (!brushAbioticMode){
            ...
            switch(tabs.getSelectionModel().getSelectedItem().getText()){
                    case("Environment (Elevation)"):
                        forestElevationView.setSelectedSpeciesSupplier(getSelectedSpecies);
                        brushAbiotic(forestElevationView, brushSizeSlider, tempSlider, sunSlider, moistSlider, false);
                        break;
                    case("Temperature"):
                        forestTemperatureView.setSelectedSpeciesSupplier(getSelectedSpecies);
                        brushAbiotic(forestTemperatureView, brushSizeSlider, tempSlider, sunSlider, moistSlider, true);
                        break;
                    case("Sunlight"):
                        forestSunlightView.setSelectedSpeciesSupplier(getSelectedSpecies);
                        brushAbiotic(forestSunlightView, brushSizeSlider, tempSlider, sunSlider, moistSlider, true);
                        break;
                    case("Moisture"):
                        forestMoistureView.setSelectedSpeciesSupplier(getSelectedSpecies);
                        brushAbiotic(forestMoistureView, brushSizeSlider, tempSlider, sunSlider, moistSlider, true);
                        break;
                    case("Age"):
                        forestAgeView.setSelectedSpeciesSupplier(getSelectedSpecies);
                        brushAbiotic(forestAgeView, brushSizeSlider, tempSlider, sunSlider, moistSlider, false);
                        break;
                    case("Pink Noise"):
                        break;
                    case("Forest"):
        ...
        enableAbioticChangeBtn.setPrefWidth(400);
        // menu for abiotics
    ...
    private void brushRemoval(ForestOnMapView mapView, Slider brushSizeSlider){
        ...
        if (brushRemovalMode){
            console.log("\nBrush Removal Mode: Toggled On. Other brush modes toggled off.");
            ...
            console.log("\nBrush Removal Mode: Toggled Off.\nNumber of Remaining Plants: " + currentResult.forest().getAllPlants().size());
        }
        
    }

    private void brushAge(ForestOnMapView mapView, Slider brushSlider, Slider ageSlider, boolean onRelevantTab){
        brushAgeMode = !brushAgeMode;
        if(brushAgeMode){
            console.log("\nBrush Age Mode: Toggled On. Other brush modes toggled off.");
            mapView.enableBrushAgeMode((() -> brushSlider.getValue()), (() -> ageSlider.getValue()), onRelevantTab, simulationEngine);
            updateBrushCursor(mapView, brushSlider.getValue());
        }else{
            mapView.disableBrushMode();
            mapView.setCursor(Cursor.DEFAULT);  
            console.log("\nBrush Age Mode: Toggled Off.");
        }
    }

    private void brushAbiotic(ForestOnMapView mapView, Slider brushSlider, Slider tempSlider, Slider sunSlider, Slider moistSlider, boolean onRelevantTab){
        brushAbioticMode = !brushAbioticMode;
        if(brushAbioticMode){
            console.log("\nBrush Abiotic Mode: Toggled On.Other brush modes toggled off.");
            mapView.enableBrushAbioticMode((() -> brushSlider.getValue()), (() -> tempSlider.getValue()), (() -> sunSlider.getValue()), (() -> moistSlider.getValue()), onRelevantTab, simulationEngine);
            updateBrushCursor(mapView, brushSlider.getValue());
        }else{
            mapView.disableBrushMode();
            mapView.setCursor(Cursor.DEFAULT);
            console.log("\nBrush Abiotic Mode: Toggled Off.");
        }
    }
...
//basic crosshair cursor but no visual resize shown but functionally yes. 
    ...
    private void removeSpecies(SimulationResult result, Map<String, CheckBox> speciesCheck){
        ...
        String out = "The following species are visible (if they exists): ";
        for (CheckBox boxes : speciesCheck.values()) {
